...
describe('workspace download proxy', function () {
  ...
  beforeEach(async function () {
    ...
    app.use(workspaceDownloadProxy.MOUNT_PATH,
      workspaceDownloadProxy.createWorkspaceDownloadProxy(
        'http://127.0.0.1:' + upstreamPort + '/services/WorkspaceDownload',
        { timeout: 1000 }
      ));
  ...
  test('requires and forwards authorization for cookie authentication', async function () {
    var missing = await request(serverPort, '/services/WorkspaceDownload/set-cookie-auth', {
      method: 'POST'
    });
    ...
    var missingWithSlash = await request(
      serverPort,
      '/services/WorkspaceDownload/set-cookie-auth/',
      { method: 'POST' }
    );
    expect(missingWithSlash.status).toBe(401);

    upstreamHandler = function (req, res) {
      expect(req.url).toBe('/services/WorkspaceDownload/set-cookie-auth');
      expect(req.headers.authorization).toBe('Bearer secret-token');
      res.statusCode = 204;
      res.end();
    };
    var authorized = await request(serverPort, '/services/WorkspaceDownload/set-cookie-auth', {
      method: 'POST',
  ...
  test('rewrites multiple cookies for an HTTP application origin', async function () {
    upstreamHandler = function (req, res) {
    ...
    var response = await request(serverPort, '/services/WorkspaceDownload/set-cookie-auth', {
      method: 'POST',
      headers: { Authorization: 'Bearer token' }
    });

    expect(response.headers['set-cookie']).toEqual([
      'session=one; Path=/services/WorkspaceDownload; HttpOnly; SameSite=Lax; Max-Age=60',
      'preference=two; Expires=Wed, 21 Oct 2030 07:28:00 GMT; Path=/services/WorkspaceDownload; SameSite=Lax'
    ]);
  });

  test('retains Secure for an effective HTTPS request', async function () {
    upstreamHandler = function (req, res) {
  ...
  test('forwards cookies, encoded paths, queries, status, headers, and body', async function () {
    ...
    upstreamHandler = function (req, res) {
      expect(req.url).toBe('/services/WorkspaceDownload/view/user/file%20%2525%23.txt?download=1&x=a%26b');
    ...
    var response = await request(serverPort,
      '/services/WorkspaceDownload/view/user/file%20%2525%23.txt?download=1&x=a%26b',
      {
  ...
  test.each([
    '/services/WorkspaceDownload/view/%2e%2e/private',
    '/services/WorkspaceDownload/view/%252e%252e/private',
    '/services/WorkspaceDownload/view/%5cprivate'
