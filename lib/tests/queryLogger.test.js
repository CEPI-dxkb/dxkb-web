...
describe('queryLogger middleware', function () {
  ...
  test('redacts authorization tokens from URL query strings', function () {
    ...
    runMiddleware(session.id, null, {
      ...
      query: { http_authorization: 'owner-token' },
    ...
    expect(appendFile.mock.calls[0][1]).not.toContain('owner-token');
  });
  ...
  test('redacts authorization tokens from POST bodies', function () {
    ...
    runMiddleware(session.id, null, {
      body: Buffer.from('query=eq(id,1)&http%5Fauthorization=owner-token&limit=10'),
      headers: {},
      method: 'POST',
      originalUrl: '/genome/',
      query: {},
